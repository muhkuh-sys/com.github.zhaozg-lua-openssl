...
def main(argv):
    ...
    for strInFile in aOptions.infile:
        ...
        strOutput = tProc.communicate()[0].decode(
            "utf-8",
            "replace"
        )
        if tProc.returncode != 0:
            raise Exception(
                'The command failed with return code %d: %s' % (
                    tProc.returncode, ' '.join(aCmd)
                )
            )
        ...
        for tMatch in re.finditer(r'DLL Name:\s+(.+).dll', strOutput):
            ...
            if strDll not in astrAllowedDlls:
                raise Exception(
                    'The DLL %s is not part of the standard system DLLs.' %
                    strDll
                )
