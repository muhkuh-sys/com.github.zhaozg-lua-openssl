...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-lua-openssl-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-lua-openssl-*.xml'))
if len(astrMatch) == 0:
    raise Exception('No match found for "lua5.4-lua-openssl-*.xml".')
elif len(astrMatch) > 1:
    raise Exception('More than one match found for "lua5.4-lua-openssl-*.xml". Clean your build folder.')
